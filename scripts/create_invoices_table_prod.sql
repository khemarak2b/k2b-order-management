-- Create invoices table for Australian business (PRODUCTION)
-- Supports GST, multiple currencies, payment tracking, and audit requirements

CREATE TABLE IF NOT EXISTS prod.invoices (
    -- Core identifiers
...
CREATE INDEX idx_invoices_order_id ON prod.invoices(order_id);
CREATE INDEX idx_invoices_user_id ON prod.invoices(user_id);
CREATE INDEX idx_invoices_invoice_number ON prod.invoices(invoice_number);
CREATE INDEX idx_invoices_status ON prod.invoices(status);
CREATE INDEX idx_invoices_payment_status ON prod.invoices(payment_status);
CREATE INDEX idx_invoices_issued_at ON prod.invoices(issued_at);
CREATE INDEX idx_invoices_due_date ON prod.invoices(due_date);
CREATE INDEX idx_invoices_created_at ON prod.invoices(created_at);
CREATE INDEX idx_invoices_deleted_at ON prod.invoices(deleted_at); -- For soft deletes

-- Create composite index for common queries
CREATE INDEX idx_invoices_user_status ON prod.invoices(user_id, status) WHERE deleted_at IS NULL;
CREATE INDEX idx_invoices_order_user ON prod.invoices(order_id, user_id);

-- Trigger to auto-generate invoice number
CREATE OR REPLACE FUNCTION prod.generate_invoice_number()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.invoice_number IS NULL THEN
        NEW.invoice_number = TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-' || 
                             LPAD(NEXTVAL('prod.invoice_number_seq')::TEXT, 5, '0');
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEFORE INSERT ON prod.invoices
FOR EACH ROW
EXECUTE FUNCTION prod.generate_invoice_number();

-- Trigger to auto-update updated_at timestamp
CREATE OR REPLACE FUNCTION prod.update_invoices_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
BEFORE UPDATE ON prod.invoices
FOR EACH ROW
EXECUTE FUNCTION prod.update_invoices_updated_at();

-- Trigger to auto-calculate amount_due
CREATE OR REPLACE FUNCTION prod.update_invoices_amount_due()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON prod.invoices
FOR EACH ROW
EXECUTE FUNCTION prod.update_invoices_amount_due();

-- Create sequence for invoice numbers (starts from 1000)
CREATE SEQUENCE IF NOT EXISTS prod.invoice_number_seq START 1000;
